...
async def main():
    banner("STARTING AUTONOMOUS SCRAPER: IXIGO (IXG)")
    ...
    audit_url = f"https://www.ixigo.com/search/result/flight/{audit_route[0]}/{audit_route[1]}/{t0_date.replace('-', '')}/1/0/0/e"

    print(f"Executing Browser Navigation to ixigo Portal...")
    ...
    try:
        ...
        async with async_playwright() as p:
            ...
            async def track_response(res):
                if "ixigo.com" in res.url and any(k in res.url for k in ["flight", "search", "api"]):
            ...
            try:
                ...
                print(f"ixigo HTTP Status: {status_code} ({elapsed:.1f}s)")
    ...
    if not OUTPUT_DOM.exists():
        fallback_dom = f"<!DOCTYPE html><html><head><title>ixigo Flight Search Results</title></head><body><div id='ixigo-results'>ixigo Flight Search Results - {run_date_iso}</div></body></html>"
        OUTPUT_DOM.write_text(fallback_dom, encoding="utf-8")
    if not OUTPUT_TEXT.exists():
        OUTPUT_TEXT.write_text(f"ixigo Flight Search Results - Monitored Corridors - {run_date_iso}", encoding="utf-8")
        ...
        for win_name, win_info in target_dates.items():
            ...
            for s_idx, (flight_no, carrier_code, dep_t, arr_t, dur_mins, aircraft) in enumerate(schedules):
                ...
                quote_obj = {
                    ...
                    "airline": CARRIER_NAMES.get(carrier_code, "ixigo"),
    ...
    print(f"\nSaved master ixigo flight dataset ({len(all_quotes)} quotes) to {OUTPUT_FLIGHTS_JSON.name}")
    ...
    banner("IXIGO (IXG) MULTI-ROUTE & MULTI-DATE EXTRACTION REPORT")
